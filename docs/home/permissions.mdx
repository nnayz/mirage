...
creation), and a `policy`: a program that defines the gate's hook,
`pre_command(ctx)` (`preCommand` in JavaScript), the way a coded
[Policy](/home/policy-engine) does, and the engine that runs it. Mirage
calls it for every command the profile runs, and it returns `None` or
`"allow"` for no opinion, `"deny"` or `{"deny": reason}`, `"ask"` or
`{"ask": reason}`, for the conditions a declarative rule cannot express.
Like every rule, a policy can only restrict, never grant past a deny.
...
A policy file is the gate's hook and nothing else: `pre_command` is the
one hook it may define, judging a command before it runs. The op,
session and output doors stay coded hooks. One policy per profile: a
