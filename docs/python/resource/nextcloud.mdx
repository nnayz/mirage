...
endpoint) at some prefix such as `/nc/`. Reads go directly over HTTP with
`aiohttp` for true async streaming, including HTTP `Range` requests for
partial reads.
...
- **Reads**: chunked via `aiohttp` `iter_chunked(8192)`. Downstream
  commands like `head -n 1` early-cancel, so you only pay for the first
  chunk over the wire.
- **Range reads**: `head -c BYTES` and other partial reads use HTTP
  `Range: bytes=...`, which Nextcloud and most WebDAV servers honor.
- **Writes**: currently buffer the full payload before `PUT`. Streaming
  uploads (chunked transfer encoding, or Nextcloud's `uploads/` resumable
  protocol) are a known follow-up.
